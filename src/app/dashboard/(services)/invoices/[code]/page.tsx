...
import { InvoiceItemUi, findInvoiceDetail } from '../lib/api';
...
export default async function Page({ params }: { params: Promise<{ code: string }> }) {
  ...
  const invoiceDetail = await findInvoiceDetail({ code }).catch((error: unknown) => {
  ...
  if (invoiceDetail && 'kind' in invoiceDetail && invoiceDetail.kind === 'error') {
    return <BackendRequestError description={invoiceDetail.message} />;
  }
    ...
    <div className='h-full flex-1 flex-col space-y-6 p-6 md:flex'>
      <div className='flex items-start justify-between gap-4'>
        ...
        <div className='flex gap-10'>
          <div>
            <DownloadExcelButton2 invoiceDetails={invoiceDetails} label={t('buttons.download')} />
          </div>
          <div>
      ...
      <div className=''>
        <div className='space-y-4 p-4'>
          <div className='grid grid-cols-1 gap-4 lg:grid-cols-3'>
            <Card className='flex flex-col rounded-xl'>
              <CardHeader className='pb-2'>
                <CardTitle className='text-sm font-semibold text-emerald-800'>{'INFORMACIÓN DE FACTURA'}</CardTitle>
              </CardHeader>
                ...
                <div className='flex h-full flex-col justify-end'>
                  ...
                  <div className='flex justify-end'>
                    <Button size='sm' variant='outline' asChild>
                      <Link scroll={false} href={`/dashboard/invoices/${code}/edit-invoice-info`}>
                        Editar
                      </Link>
                    </Button>
                  </div>
            ...
            <Card className='rounded-xl'>
              <CardHeader className='pb-2'>
                <CardTitle className='text-sm font-semibold text-emerald-800'>{'INFORMACIÓN DE PROVEEDOR'}</CardTitle>
              </CardHeader>
              <CardContent className='space-y-3 pt-0'>
                <div className='text-sm'>
                  <div className='mb-1 text-muted-foreground'>{'VINCULACIÓN'}</div>
                ...
                <div className='text-sm'>
                  <div className='mb-1 text-muted-foreground'>{'CONDICIÓN'}</div>
                ...
                <div className='flex h-full flex-col justify-end'>
                  ...
                  <div className='flex justify-end'>
                    <Button size='sm' variant='outline' asChild>
                      <Link scroll={false} href={`/dashboard/invoices/${code}/edit-supplier-info`}>
                        Editar
                      </Link>
                    </Button>
                  </div>
            ...
            <Card className='flex flex-col rounded-xl'>
              ...
              <CardContent className='flex flex-1 flex-col space-y-3 pt-0'>
                <div className='text-sm'>
                  <div className='mb-1 text-muted-foreground'>{'FORMA DE PAGO'}</div>
                ...
                <div className='flex h-full flex-1 flex-col justify-end'>
                  ...
                  <div className='flex justify-end'>
                    <Button size='sm' variant='outline' asChild>
                      <Link scroll={false} href={`/dashboard/invoices/${code}/edit-transaction-info`}>
                        Editar
                      </Link>
                    </Button>
                  </div>
          ...
          <Card className='rounded-xl'>
            <CardHeader className='pb-2'>
              <CardTitle className='text-sm font-semibold text-emerald-800'>{'REPRESENTANTE LEGAL'}</CardTitle>
            </CardHeader>
            <CardContent className='pt-0'>
              ...
              <div className='mt-6 flex h-full flex-1 flex-col justify-end'>
                {/* <div className='flex justify-end'>
                  <Button size="sm" variant="outline" onClick={handlerEditLegalRepresentativeInfo}>Editar</Button>
                </div> */}
                <div className='flex justify-end'>
                  <Button size='sm' variant='outline' asChild>
                    <Link scroll={false} href={`/dashboard/invoices/${code}/edit-legal-representative-info`}>
                      Editar
                    </Link>
                  </Button>
                </div>
          ...
          <Card className='rounded-xl'>
            <CardHeader className='pb-2'>
              <CardTitle className='text-sm font-semibold text-emerald-800'>{'DETALLE DE ÍTEMS'}</CardTitle>
            </CardHeader>
            <CardContent className='pt-0'>
                  ...
                  <Table className='min-w-max overflow-x-auto'>
                    <TableHeader>
                      <TableRow className='bg-[#e6f2ee]'>
                        ...
                        <TableHead className='sticky right-0 z-30 min-w-[140px] whitespace-nowrap bg-[inherit] text-center'>
                    ...
                    <TableBody>
                      {invoiceDetails.items.map((it) => (
                        <TableRow key={it.id}>
                          ...
                          <TableCell className='sticky right-0 z-10 min-w-[140px] bg-background px-2 group-hover:bg-muted/50'>
                            ...
                            <div className='relative flex justify-center'>
                              ...
                              <Button size='sm' className='flex w-24 min-w-24' variant='outline' asChild>
                                <Link
                                  scroll={false}
                                  className='w-24'
                                  href={`/dashboard/invoices/${code}/edit-invoice-item/${it.id}`}
                                >
                                  Editar
                                </Link>
                              </Button>
