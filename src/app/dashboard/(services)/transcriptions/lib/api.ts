...
import { getBackendAccessToken, getSession } from '@/lib/session';

import { BackendApiError } from './backend-error';
import {
...
const backendFetch = async <T>(path: string, options: RequestOptions = {}): Promise<T> => {
  ...
  nextDebugLog('invoices.backendFetch', 'request:start', {
    path,
    method,
    hasAccessToken: Boolean(accessToken),
    bodyType: options.body instanceof FormData ? 'form-data' : options.body ? 'json' : 'empty',
  });

  const response = await fetch(buildBackendUrl(path), init);
  const data = await readResponse(response);

  nextDebugLog('invoices.backendFetch', 'response:received', {
    path,
    method,
  ...
  if (response.status === 404 && options.notFoundAsNull) {
    nextDebugLog('invoices.backendFetch', 'response:not-found-as-null', {
      path,
      method,
    });
    return null as T;
  ...
  if (!response.ok) {
    nextDebugError('invoices.backendFetch', 'response:error', {
      path,
      method,
      status: response.status,
      message: getErrorMessage(data, `Backend request failed with status ${response.status}`),
    });
