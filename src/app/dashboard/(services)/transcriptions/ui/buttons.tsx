...
import { generateDefaultReportAction } from '../actions';
...
export function DownloadVerifications() {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await generateDefaultReportAction();
      ...
      channel.bind('report-generated', (data: any) => {
        toast.custom(
          () => <ReporteToast reportId={data.reportId} url={data.url.value} duration={data.expiresIn * 1000} />,
          {
...
export function ViewVerification({ code }: { code: string }) {
  return (
    <Button variant='link' asChild>
      <Link href={`/dashboard/invoices/${code}`}>
